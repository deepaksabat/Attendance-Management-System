...
@section('content')
    <div>
        <ul class="breadcrumb">
            ...
            <li>
                <a href="{!! URL::to('company/chat') !!}">Chat</a>
            </li>
        </ul>
    </div>
    <div>
        ...
        $(document).ready(function () {
            ...
            $(".chat-content").bind("scroll", function() {
                ...
                if($(this).scrollTop() == 0){
                    ...
                    $.ajax({
                        url: "{!! URL::to('company/message-more') !!}",
                        type: "POST",
...
//            console.log($scope.Usermessages);
            $scope.messageSave = function(event){
                event.preventDefault();
                    var req = {
                        method: 'POST',
                        url: "{!! URL::to('company/message-save') !!}",
            ...
            $scope.messageCheck = function (id) {
                if ($scope.Usermessages[id].length == 0) {
                    var req = {
                        method: 'GET',
                        url: '{!! URL::to("company/check-message/") !!}/' + id,
                        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                        data: ''
                    };
                    $http(req).success(function (response) {
                    ...
                    var req = {
                        method: 'GET',
                        url: '{!! URL::to("company/message-mark-read/") !!}/' + id,
                        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                        data: ''
                    };
                    $http(req).success(function (response) {
